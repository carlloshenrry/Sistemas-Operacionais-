...
int main(int argc, char * argv[]) {
    ...
    if (semop(g_sem_id, g_sem_op1, 1) == -1) { //inicializa com semafaro destrancado ->op2
     ...
     */
    if ((g_shm_id = shmget(SHM_KEY, sizeof(int), IPC_CREAT | 0666)) == -1) { //alterado para 0666
...
void PrintChars(void) {
    ...
    while (1) {
        ...
        *g_shm_addr = tmp_index + i;
        ...
        #ifdef PROTECT
        if (semop(g_sem_id, g_sem_op1, 1) == -1) { // Realiza destranca com o (g_sem_op2) 		 
